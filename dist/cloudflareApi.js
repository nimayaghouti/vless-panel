...
async function cfFetch(token, path, init = {}) {
    ...
    if (!data.success) {
        const message = data.errors?.[0]?.message ?? `Cloudflare API error (HTTP ${res.status})`;
        throw new Error(message);
    }
    return data.result;
}
async function uploadWorker(options) {
    const bindings = [
    ...
    form.append('worker.js', new Blob([options.workerSource], { type: 'application/javascript+module' }), 'worker.js');
    await cfFetch(options.token, `/accounts/${options.accountId}/workers/scripts/${options.scriptName}`, {
        method: 'PUT',
        body: form,
    });
}
async function enableWorkersDevRoute(options) {
    await cfFetch(options.token, `/accounts/${options.accountId}/workers/scripts/${options.scriptName}/subdomain`, {
        method: 'POST',
...
async function getAccountSubdomain(options) {
    const result = await cfFetch(options.token, `/accounts/${options.accountId}/workers/subdomain`);
    return result.subdomain;
}
export function buildVlessLink(uuid, hostname, remark) {
...
export async function getTodayRequestCount(token, accountId, scriptName) {
    const now = new Date();
